...
export default function CareersPage() {
    ...
    return (
        <main className="mt-10 lg:mt-18 min-h-screen">
            {/* Hero Image Section */}
            <section className="relative bg-white w-full h-[60vh] md:h-[95vh]">
                ...
                <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-center items-center text-center z-10 px-4 sm:px-6">
                    <h1 className="
                        text-white 
                        text-2xl 
                        sm:text-4xl 
            ...
            {/* Our Culture Section */}
            <article className="flex flex-col items-center justify-center px-4 sm:px-8 md:px-16 lg:px-24 xl:px-30 py-8 sm:py-10 md:py-12 bg-[#F9FAFB] text-center">
                <h2 className="text-2xl sm:text-3xl md:text-3xl lg:text-5xl xl:text-5xl font-bold text-[#144afc] mb-2 md:mb-6">
                    A Culture That Inspires
                </h2>
                <p className="text-left text-base sm:text-lg md:text-lg xl:text-2xl text-gray-700 leading-relaxed">
            ...
            <article className="flex flex-col-reverse md:flex-row items-center justify-center gap-2 md:gap-4  px-1 md:3 py-3 md:py-6 text-left rounded-xl mx-2 sm:mx-6 md:mx-12 lg:mx-20 max-w-full">
                <div className="w-full md:w-1/2 flex flex-col gap-5 items-center md:items-start">
                    <h3 className="text-2xl md:text-3xl lg:text-5xl font-bold text-[#144afc] drop-shadow-lg mb-2">
                        Career Opportunities
                    </h3>
                    <p className="text-base sm:text-lg md:text-lg xl:text-2xl text-black/90 leading-relaxed">
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 px-1 py-10 md:px-5 bg-[#F9FAFB]">
                {careerValues.map((item, index) => (
                    <div
                        key={index}
                        className="flex flex-col bg-[#F1F5FF] rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 overflow-hidden border border-gray-200"
                    >
                        <Image
                            src={item.image}
                            alt={item.title}
                            ...
                            unoptimized
                        />
                        <div className="p-2 md:p-5 flex flex-col gap-3 flex-1">
                          
                            <h2 className="text-xl sm:text-2xl xl:text-3xl font-semibold text-gray-800">{item.title}</h2>
                            <p className="text-gray-600 text-sm sm:text-base xl:text-xl">{item.description}</p>
                        </div>
                    </div>
                ))}
            </section>
            ...
            <article className="flex flex-col justify-center items-center w-full ">
                <div className="flex flex-col gap-2 md:gap-4 lg:gap-6 justify-center items-center px-3 py-2 md:py-4 lg:py-6 sm:px-6 sm:py-10 md:px-8  text-black max-w-full text-center">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl xl:text-4xl font-semibold text-[#144afc]">
                        Apply Now
                    </h1>
                    <p className="text-base sm:text-lg md:text-xl xl:text-2xl text-left">
                    ...
                    <Link
                        href="/contact"
                        className="mt-4 px-6 py-2 sm:px-8 sm:py-3 bg-[#005fcc] text-white rounded-2xl font-semibold text-base sm:text-lg xl:text-xl hover:bg-[#004799] transition-colors"
                    >
                        Contact Us
                    </Link>
